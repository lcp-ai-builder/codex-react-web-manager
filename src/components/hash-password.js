const subtle = globalThis.crypto?.subtle || globalThis.crypto?.webcrypto?.subtle;
const textEncoder = typeof TextEncoder !== 'undefined' ? new TextEncoder() : null;

const toHex = (bytes) =>
  Array.from(bytes)
    .map((byte) => byte.toString(16).padStart(2, '0'))
    .join('');

const rightRotate = (value, amount) => (value >>> amount) | (value << (32 - amount));

const sha256Fallback = (value) => {
  // Fallback implementation for environments without Web Crypto (e.g. non-secure origins).
  const k = [
    0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5, 0x3956c25b, 0x59f111f1, 0x923f82a4,
    0xab1c5ed5, 0xd807aa98, 0x12835b01, 0x243185be, 0x550c7dc3, 0x72be5d74, 0x80deb1fe,
    0x9bdc06a7, 0xc19bf174, 0xe49b69c1, 0xefbe4786, 0x0fc19dc6, 0x240ca1cc, 0x2de92c6f,
    0x4a7484aa, 0x5cb0a9dc, 0x76f988da, 0x983e5152, 0xa831c66d, 0xb00327c8, 0xbf597fc7,
    0xc6e00bf3, 0xd5a79147, 0x06ca6351, 0x14292967, 0x27b70a85, 0x2e1b2138, 0x4d2c6dfc,
    0x53380d13, 0x650a7354, 0x766a0abb, 0x81c2c92e, 0x92722c85, 0xa2bfe8a1, 0xa81a664b,
    0xc24b8b70, 0xc76c51a3, 0xd192e819, 0xd6990624, 0xf40e3585, 0x106aa070, 0x19a4c116,
    0x1e376c08, 0x2748774c, 0x34b0bcb5, 0x391c0cb3, 0x4ed8aa4a, 0x5b9cca4f, 0x682e6ff3,
    0x748f82ee, 0x78a5636f, 0x84c87814, 0x8cc70208, 0x90befffa, 0xa4506ceb, 0xbef9a3f7,
    0xc67178f2,
  ];

  let h0 = 0x6a09e667;
  let h1 = 0xbb67ae85;
  let h2 = 0x3c6ef372;
  let h3 = 0xa54ff53a;
  let h4 = 0x510e527f;
  let h5 = 0x9b05688c;
  let h6 = 0x1f83d9ab;
  let h7 = 0x5be0cd19;

  const encodeUtf8 = (input) => {
    if (textEncoder) return textEncoder.encode(input);
    const codePoints = [];
    for (let i = 0; i < input.length; i += 1) {
      const code = input.charCodeAt(i);
      if (code < 0x80) {
        codePoints.push(code);
      } else if (code < 0x800) {
        codePoints.push(0xc0 | (code >> 6));
        codePoints.push(0x80 | (code & 0x3f));
      } else {
        codePoints.push(0xe0 | (code >> 12));
        codePoints.push(0x80 | ((code >> 6) & 0x3f));
        codePoints.push(0x80 | (code & 0x3f));
      }
    }
    return new Uint8Array(codePoints);
  };

  const message = encodeUtf8(value);
  const bitLength = message.length * 8;
  const withPadding = new Uint8Array(((message.length + 9 + 63) >> 6) << 6);
  withPadding.set(message);
  withPadding[message.length] = 0x80;

  const view = new DataView(withPadding.buffer);
  view.setUint32(withPadding.length - 8, Math.floor(bitLength / 0x100000000), false);
  view.setUint32(withPadding.length - 4, bitLength >>> 0, false);

  const words = new Uint32Array(64);

  for (let i = 0; i < withPadding.length; i += 64) {
    for (let j = 0; j < 16; j += 1) {
      words[j] = view.getUint32(i + j * 4, false);
    }
    for (let j = 16; j < 64; j += 1) {
      const s0 =
        rightRotate(words[j - 15], 7) ^ rightRotate(words[j - 15], 18) ^ (words[j - 15] >>> 3);
      const s1 =
        rightRotate(words[j - 2], 17) ^ rightRotate(words[j - 2], 19) ^ (words[j - 2] >>> 10);
      words[j] = (words[j - 16] + s0 + words[j - 7] + s1) >>> 0;
    }

    let a = h0;
    let b = h1;
    let c = h2;
    let d = h3;
    let e = h4;
    let f = h5;
    let g = h6;
    let h = h7;

    for (let j = 0; j < 64; j += 1) {
      const s1 = rightRotate(e, 6) ^ rightRotate(e, 11) ^ rightRotate(e, 25);
      const ch = (e & f) ^ (~e & g);
      const temp1 = (h + s1 + ch + k[j] + words[j]) >>> 0;
      const s0 = rightRotate(a, 2) ^ rightRotate(a, 13) ^ rightRotate(a, 22);
      const maj = (a & b) ^ (a & c) ^ (b & c);
      const temp2 = (s0 + maj) >>> 0;

      h = g;
      g = f;
      f = e;
      e = (d + temp1) >>> 0;
      d = c;
      c = b;
      b = a;
      a = (temp1 + temp2) >>> 0;
    }

    h0 = (h0 + a) >>> 0;
    h1 = (h1 + b) >>> 0;
    h2 = (h2 + c) >>> 0;
    h3 = (h3 + d) >>> 0;
    h4 = (h4 + e) >>> 0;
    h5 = (h5 + f) >>> 0;
    h6 = (h6 + g) >>> 0;
    h7 = (h7 + h) >>> 0;
  }

  const result = new Uint8Array(32);
  const hashParts = [h0, h1, h2, h3, h4, h5, h6, h7];

  for (let i = 0; i < hashParts.length; i += 1) {
    result[i * 4] = hashParts[i] >>> 24;
    result[i * 4 + 1] = (hashParts[i] >>> 16) & 0xff;
    result[i * 4 + 2] = (hashParts[i] >>> 8) & 0xff;
    result[i * 4 + 3] = hashParts[i] & 0xff;
  }

  return toHex(result);
};

const hashWithSubtle = async (value) => {
  if (!textEncoder) {
    throw new Error('TextEncoder unavailable for Web Crypto hashing');
  }
  const data = textEncoder.encode(value);
  const hashBuffer = await subtle.digest('SHA-256', data);
  return toHex(new Uint8Array(hashBuffer));
};

export const hashPassword = async (value) => {
  if (subtle) {
    try {
      return await hashWithSubtle(value);
    } catch (error) {
      console.warn('Falling back to JS SHA-256 implementation', error);
    }
  }
  return sha256Fallback(value);
};
